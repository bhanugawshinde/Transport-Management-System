...
public class AddManager extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ManagerBean mg = new ManagerBean(firstName, lastName, email, phoneNumber, dateOfBirth, hireDate, department, address, city, state, postalCode, country, null, branchId, salary);
		// Response
		JsonObject jsonResponse = new JsonObject();
		resp.setContentType("application/json");
		try (PrintWriter out = resp.getWriter()) {
		if (ManagerDao.add(mg)) {
